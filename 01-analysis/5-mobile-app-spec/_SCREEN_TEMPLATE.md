...
**Дизайн-макет:** {Ссылка на Figma или «На основе 3-design-brief»}
...
**Спецификация:** [`api/{domain}.yaml`](../api/{domain}.yaml) → `{operationId}`
...
| HTTP 5xx | — | Снек "Что-то пошло не так. Попробуйте позже" |
| Сеть | Нет соединения | Снек "Нет соединения. Проверьте подключение" |

---

**Доступные спецификации (проект «Шеф-стол»):**

REST API (`api/`):
- `auth/api.yaml` — авторизация (OTP)
- `profile/api.yaml` — профиль клиента
- `slots/api.yaml` — список/фильтрация классов
- `bookings/api.yaml` — бронирования
- `chefs/api.yaml` — справочник шефов
...
| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Поле "Имя*" | Имя клиента | `name` из кэша профиля | Кириллица/латиница, 2-50 символов. Ошибка: "Имя имеет неверный формат" | — |
| Поле "Телефон*" | Номер телефона | `phone` из кэша профиля | Формат E.164. Ошибка: "Телефон имеет неверный формат" | — |
| Кнопка "Сохранить" | Primary button | — | — | Валидация → [{actionEndpoint}](#actionendpoint) |

**Момент валидации:** {При потере фокуса / При отправке формы}

**Логика:**
- Кнопка "Сохранить": При тапе → валидация всех полей → при успехе отправить запрос [{actionEndpoint}](#actionendpoint)

**Условия доступности:**
- Кнопка "Сохранить" активна, если: все обязательные поля заполнены И валидация пройдена
...
|----|----------|-----------|
| FR-XXX | {Название} | {Must/Should} |

### Нефункциональные (NFR-*)

| ID | Название | Приоритет |
|----|----------|-----------|
| NFR-XXX | {Название} | {Высокий/Средний} |
