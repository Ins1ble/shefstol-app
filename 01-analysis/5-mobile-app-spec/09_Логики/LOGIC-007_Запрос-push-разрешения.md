...
> и на локальный флаг «запрос разрешения уже показывался». Текст подводки зависит от
> `reminder_hours` — массива часов напоминания (тоже из ответа `createBooking`). Доставку push
> обеспечивает существующая инфраструктура; приложение через клиентский REST API «Шеф-стол»
> **регистрирует и удаляет push-токен** (`registerPushToken` / `deletePushToken`).

| Название | Тип | Возможные значения | Описание |
|----------|-----|-------------------|----------|
| `is_first_booking` | Поле ответа `createBooking` | `true` / `false` | Признак того, что только что созданная бронь — **первая успешная запись** клиента. Источник — `CreateBookingResponse.is_first_booking`; управляет показом запроса push-разрешения на [BS-002](../BS-002-booking-success.md). |
...
| `reminder_hours` | Поле ответа `createBooking` | массив целых, напр. `[24, 2]` | За сколько часов до начала класса приходят **повторные напоминания**. Из `CreateBookingResponse.reminder_hours`. Подставляется в текст подводки («Напомним за N часов до начала»); пусто/отсутствует — напоминания не настроены. **Не хардкодится.** |
...
показывается **только после первой успешной записи на кулинарный класс** — на экране подтверждения
[BS-002](../BS-002-booking-success.md), когда ценность напоминаний очевидна
(«напомним за N часов до начала»), а не на старте приложения и не в профиле.
...
**регистрирует его в API «Шеф-стол»** запросом `registerPushToken`
(`POST /auth/push-tokens`); при выходе/смене устройства токен снимается `deletePushToken`.
Саму доставку напоминаний и уведомлений об отмене класса обеспечивает инфраструктура (FR-33, NFR-13).

**Контракт push-токена и повторные напоминания.** Регистрация токена — `registerPushToken`
...
> Как клиент кулинарной студии, я хочу после первой записи разрешить напоминания о классе,
> чтобы не забыть и не пропустить начало, — но без навязчивых запросов, если я отказался.

### Бизнес-ценность

- Снижение неявок: заблаговременное напоминание о предстоящем классе (US-12, NFR-13).
...
|-----------------|-----------------|---------|
| [BS-002 Подтверждение записи](../BS-002-booking-success.md) | После показа сводки брони — подводка «включить напоминания» → системный запрос разрешения | Только если `is_first_booking = true` **и** `push_permission_requested = false` |
...
flowchart TD
    ...
    Flag -->|Да| Hint["Подводка: «Напомним за N часов до начала»"]
...
**«Напомним за N часов до начала»**, где часы берутся из `reminder_hours` (массив из ответа
...
(APNs / FCM) и **регистрирует его в API «Шеф-стол»** запросом `registerPushToken`
(`POST /auth/push-tokens`, тело `{ token, platform }`). Повторная регистрация того же
токена идемпотентна. Дальнейшую доставку напоминаний (за `reminder_hours` часов до начала) и
...
push-токен не регистрируется (`registerPushToken` не вызывается), экран подтверждения работает, переходы
«Мои записи»/«Готово» доступны. Запрос **не повторяется** ни в этой, ни в последующих
...
> согласии приложение **регистрирует push-токен в REST API «Шеф-стол»** (`registerPushToken`);
...
| Ошибка 400 / 401 | Тихо игнорировать в UI; при 401 — общий 401-flow (LOGIC-001). Запись/экран не страдают. |
...
|----|----------|-----------|
| FR-33 | Напоминание о предстоящей записи; доставку push обеспечивает существующая инфраструктура (приложение регистрирует push-токен) | Should |
...
| API Push | `registerPushToken`, `deletePushToken` ([`../../api/auth/api.yaml`](../../api/auth/api.yaml)) | Medium |

### UI/UX

| ID | Название |
|----|----------|
| US-12 | Получать напоминание о предстоящей записи, чтобы не забыть и не пропустить класс |

### Нефункциональные (NFR-*)

| ID | Название | Приоритет |
|----|----------|-----------|
| NFR-13 | Своевременная доставка напоминаний (заблаговременно до начала); доставку обеспечивает существующая инфраструктура | Средний |
...
| ID | Критерий |
|----|----------|
| AC-001 | **Дано** это первая успешная запись клиента (`is_first_booking = true`, `push_permission_requested = false`), **Когда** открыт экран BS-002 и показана сводка, **Тогда** после сводки показывается подводка «Напомним за N часов до начала» и вызывается системный запрос разрешения на push. |
...
| AC-004 | **Дано** показан системный запрос, **Когда** клиент **отказывает** в разрешении, **Тогда** бронь и экран работают штатно — **просто без напоминаний** (push-токен не регистрируется), переходы доступны, запрос больше **не повторяется** и нет повторных диалогов/давления (P6). |
...
| AC-007 | **Дано** значение `reminder_hours` пришло в ответе `createBooking` (массив часов, напр. `[24, 2]`), **Когда** показывается подводка к запросу, **Тогда** в тексте подставляются именно эти часы («за N часов до начала») без хардкода; если массив пуст/отсутствует — текст подводки про напоминания не показывается. |
...
| -------------------------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Отказ в разрешении (`denied`)                | Клиент отклонил системный диалог                                                               | Не блокировать запись/экран; не повторять запрос; без снека/тоста и без давления (P6). Флаг `push_permission_requested = true` уже выставлен. Включение в дальнейшем — только через системные настройки ОС (допустима подсказка-ссылка). |
