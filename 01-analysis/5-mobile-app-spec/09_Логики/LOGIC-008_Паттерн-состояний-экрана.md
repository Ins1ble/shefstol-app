...
> Как клиент кулинарной студии, я хочу на каждом экране понимать, что происходит — загружаются ли классы, нет ли записей, или произошла ошибка,
...
- Доступность на кухне/в студии: состояния дублируются текстом/иконкой/формой, считываются при ярком освещении и при дальтонизме (NFR-1).
...
| [SCR-002 Список классов](../SCR-002-slot-list.md) | При открытии вкладки, pull-to-refresh | Пример применения |
| [SCR-003 Карточка класса](../SCR-003-slot-card.md) | При открытии экрана | Пример применения |
| [SCR-005 Мои записи](../SCR-005-my-bookings.md) | При открытии вкладки, pull-to-refresh | Пример применения |
...
|----------------|-----------------|--------------------------|----------|
| Нет доступных классов вообще | «Пока нет доступных классов» | «Потяните вниз, чтобы обновить» | [SCR-002](../SCR-002-slot-list.md) |
| Нет классов по выбранным фильтрам | «Ничего не найдено по фильтрам» | CTA «Изменить фильтры» | [SCR-002](../SCR-002-slot-list.md) |
| Нет предстоящих броней | «Пока нет предстоящих записей» | CTA «Записаться на класс» → [SCR-002](../SCR-002-slot-list.md) | [SCR-005](../SCR-005-my-bookings.md) |
| Нет прошедших броней | «Здесь появятся прошедшие классы» | CTA «Записаться на класс» → [SCR-002](../SCR-002-slot-list.md) | [SCR-005](../SCR-005-my-bookings.md) |
| Адрес студии без карты (координаты отсутствуют) | «Адрес студии» (текстовый блок) + опциональная ссылка «Открыть в картах» | — (это Content-без-карты, не Error) | [BS-004](../BS-004-studio-address.md), [LOGIC-006](LOGIC-006_Адрес-студии.md) |
...
Различие состояний **не передаётся только цветом** — каждое состояние дублируется иконкой, текстом и/или формой (скелетон, заглушка, лоадер на кнопке). Это требование NFR-1 для использования при ярком освещении и при дальтонизме.
...
|----|----------|-----------|
| NFR-1 | Удобство использования: mobile-first, крупные элементы, высокий контраст; состояния дублируются не только цветом (доступность на кухне/при дальтонизме) | High |
