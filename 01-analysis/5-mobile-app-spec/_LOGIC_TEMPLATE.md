...
> **Важно:** Указывайте ссылки на документы. Если логика используется в переиспользуемом компоненте, ссылайтесь на компонент вместо перечисления всех экранов.
...
| `Idempotency-Key` | UUID v4 (только для POST /bookings) |
...
| Успех (200/201) | {Описание успешного сценария} |
...
|------|--------------|----------|
| `{key_name}` | {Локальный кэш / Защищённое хранилище (Keychain/Keystore)} | {Описание} |
